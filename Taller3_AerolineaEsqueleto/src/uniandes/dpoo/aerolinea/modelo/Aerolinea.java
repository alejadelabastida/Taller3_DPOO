...
public class Aerolinea
{
    ...
    {
        // TODO implementar
    	List<Tiquete> todosLosTiquetes = new LinkedList<Tiquete>();
    	for(Vuelo vuelo:vuelos)
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        {
            if( vueloExistente.getAvion() == avion && vueloExistente.getFecha().equals(fecha))
            {
                Ruta rutaExistente = vueloExistente.getRuta();
                int inicioExistente = Ruta.getHoras( rutaExistente.getHoraSalida()) * 60 + Ruta.getMinutos( rutaExistente.getHoraSalida( ) );
                int finExistente = Ruta.getHoras( rutaExistente.getHoraLlegada()) * 60 + Ruta.getMinutos( rutaExistente.getHoraLlegada( ) );
 
                if( inicioNuevo < finExistente && inicioExistente < finNuevo )
                    throw new Exception( "El avión " + nombreAvion + " ya está ocupado en otro vuelo el " + fecha + " a esa hora" );
            }
        }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	...
    	if(cliente == null)
    		throw new Exception("no existe un lcinete con el identificador" + identificadorCliente);
    	Vuelo vuelo = getVuelo(codigoRuta, fecha);
    	if (vuelo == null)
    		throw new Exception("no existe No existe un vuelo para la ruta" + codigoRuta + "en la fecha" + fecha);
    	CalculadoraTarifas calculadora = obtenerCalculadoraTarifas(fecha);
    	return vuelo.venderTiquetes(cliente, calculadora, cantidad);
    }
    
    private CalculadoraTarifas obtenerCalculadoraTarifas( String fecha )
    {
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
        // TODO Implementar el método
         ...
         return "El cliente " + identificadorCliente + " tiene un saldo pendiente de " + saldo;
     }
